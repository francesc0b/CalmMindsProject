...
public class viewAllCounselorsController implements Initializable {
    ...
    @FXML private TableColumn numOfClientsCol;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        numOfClientsCol.setCellValueFactory(new PropertyValueFactory<>("numOfClients"));
        ...
        try{
            String query = "SELECT p.*,c.c_id,yearsOfExp,hireDate,d.educationLevel,d.type, count(CounID) as numOfClients FROM Person p\n" +
                    "JOIN counselor c ON c.SSN = p.SSN \n" +
                    "JOIN degree d ON d.counselID = c.c_id\n" +
                    "JOIN contract co ON co.counID = c.c_id \n" +
                    "WHERE p.type='counselor' group by c_id";
            ...
            while(rs.next()){
                ...
                counselor.setNumOfClients(rs.getInt("numOfClients"));
